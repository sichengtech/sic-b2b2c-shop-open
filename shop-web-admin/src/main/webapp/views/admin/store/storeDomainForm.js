$(function () {
    ...
    if (jsValidate) {
        $("#inputForm").validate({
            rules: {
                "domain": {
                    remote: ctxa + "/store/storeDomain/validateDomain.do?oldDomain=" + encodeURIComponent(oldDomain),
                    required: true,
                    maxlength: 64,
                    minlength: 4,
            ...
            messages: {
                "domain": {
                    remote: fy.getMsg('二级域名已存在'),
                    required: fy.getMsg('必填项'),
                    maxlength: fy.getMsg('最大长度不能超过') + 64 + fy.getMsg('字符'),
                    minlength: +fy.getMsg('最小长度不能超过') + 4 + fy.getMsg('字符'),
                    regex: fy.getMsg('请输入合格的域名要求'),
                },
            },
            errorPlacement: function (error, element) {
                //错误提示信息的显示位置
                if (element.is("form-control input-sm")) {
