$(function () {
	...
	if (jsValidate) {
        $("#inputForm").validate({
            ...
            messages: {
                "no": {required: fy.getMsg('工号不能为空'), maxlength: fy.getMsg('工号不能超过') + 100 + fy.getMsg('字符'),},
                "name": {required: fy.getMsg('姓名不能为空'), maxlength: fy.getMsg('姓名不能超过') + 100 + fy.getMsg('字符'),},
                "loginName": {required: fy.getMsg('登录名不能为空'), maxlength: fy.getMsg('登录名不能超过') + 100 + fy.getMsg('字符'),},
                "newPassword": {
                    regex: fy.getMsg('密码至少') + 6 + fy.getMsg('字符'),
                    required: fy.getMsg('新密码不能为空'),
                    maxlength: fy.getMsg('密码不能超过') + 10 + fy.getMsg('字符'),
                },
                "confirmNewPassword": {
                    required: fy.getMsg('确认新密码不能为空'),
