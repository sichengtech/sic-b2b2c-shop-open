...
<html lang="zh-CN">
<head>
<title>${fns:fy("平台账户流水管理")}</title>
<meta name="keywords" content=""/>
<meta name="description" content=""/>
<meta name="decorator" content="admin"/>
<!-- 业务js -->
<script type="text/javascript" src="${ctx}/views/admin/account/accountPlatformSnList.js"></script>
</head>
<body>
	<!-- panel开始 -->
	<section class="panel">
		<header class="panel-heading custom-tab tab-right ">
			<h4 class="title">${fns:fy("平台账户流水列表")}</h4>
			<%@ include file="../include/functionBtn.jsp"%>
			<ul class="nav nav-tabs pull-right">
				<li class="active"><a href="javaScript:;"> <i class="fa fa-user"></i> ${fns:fy("平台账户流水列表")}</a></li>
			</ul>
		</header>
		...
		<div class="panel-body">
			...
			<div class="alert alert-info alert-block fade in ${cookie.fdp_operationTips.value=='0'?'point_hidden':''}" id="point">
				<h5>${fns:fy("操作提示")}</h5>
				<ul>
					<li>${fns:fy("平台账户流水管理是查看平台账户的流水信息")}</li>
				</ul>
			</div>
			<!-- 提示 end -->
			...
			<table class="table table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr>
						<td>${accountPlatformSn.apId}</td>
						<td>${accountPlatformSn.serialNumber}</td>
						<td>${not empty accountPlatformSn.incomeMoney && accountPlatformSn.incomeMoney ne '0.000'?accountPlatformSn.incomeMoney:'0'}</td>
