...
<body>
	...
	<section class="panel">
		<header class="panel-heading custom-tab tab-right ">
			<h4 class="title">${fns:fy("店铺销售")}</h4>
			<%@ include file="../include/functionBtn.jsp"%>
			<ul class="nav nav-tabs pull-right">
				<li class=""><a href="${ctxa}/report/storeSale/countDayNum.do"> <i class="fa fa-user"></i> ${fns:fy("按天统计")}</a></li>
				<li class="active"><a href="javascript:;"> <i class="fa fa-user"></i> ${fns:fy("按周统计")}</a></li>
				<li class=""><a href="${ctxa}/report/storeSale/countMonthNum.do"> <i class="fa fa-user"></i> ${fns:fy("按月统计")}</a></li>
			</ul>
		</header>
		...
		<div class="panel-body">
			...
			<div class="alert alert-info alert-block fade in ${cookie.fdp_operationTips.value=='0'?'point_hidden':''}" id="point">
				<h5>${fns:fy("操作提示")}</h5>
				<ul>
					<li>${fns:fy("报表管理.店铺销售.操作提示3")}</li>
					<li>${fns:fy("报表管理.店铺销售.操作提示2")}</li>
				</ul>
			</div>
			<!-- 提示 end --> 
			...
			<div class="row" style="margin-bottom:10px">
				...
				<form action="${ctxa}/report/storeSale/countWeekNum.do" method="get" id="searchForm">
					<div class="col-sm-3"></div>
					<div class="col-sm-2">
						<input type="text" class="form-control input-sm" placeholder="${fns:fy("店铺名称")}" name="storeName" value="${storeName}">
					</div>
					<div class="col-sm-1">
		    ...
		    <script type="text/javascript">
		        ...
		        var option = {
		        	    ...
		        	    xAxis: {
		        	    	axisLabel: {
		        	            interval: 0,
		        	            rotate: -40 //坐标字旋转角度-90到90
		        	        },
		        	        type: 'category',
		        	        data: [<c:forEach items="${list}" var="report">'${report.STORENAME}',</c:forEach>]
		        	    },
		        	    yAxis: {
			...
			<div class="table-responsive">
				...
				<tbody>
					<c:forEach items="${list}" var="report" >
						<tr>
							<td><a href="${ctxa}/store/store/list.do?name=${report.STORENAME}">${report.STORENAME}</a></td>
