			...
			<dl class="box goods-add">
				<dt class="box-header">
					<h4 class="pull-left">
						...
						<span>${fns:fy('入驻信息')}<button  href="${ctxs}/store/storeEnterInfo/reduction.htm?enterId=${storeEnterAuth.enterId}" style="margin-left: 20px" class="sui-btn btn-primary reduction">${fns:fy('入驻信息还原')}</button></span>
					...
					<ul class="sui-breadcrumb">
						...
						<li>${fns:fy('店铺')}</li>
						<li>${fns:fy('店铺管理')}</li>
						<li class="active">${fns:fy('入驻信息')}</li>
					</ul>
				</dt>
					...
					<div class="sui-msg msg-tips msg-block">
						...
						<c:if test="${storeEnterAuth.status=='70'}">
							<div class="sui-msg msg-large msg-block msg-error">
							 	<div class="msg-con" style="margin-left: -10px;width: 886px"><i class="sui-icon icon-tb-warnfill"></i>${fns:fy('当前入驻信息编辑后正在审核中，不可再次编辑。')}</div>
							</div>
						</c:if>
						<c:if test="${storeEnterAuth.status=='90'}">
							<div class="sui-msg msg-large msg-block msg-error">
							 	<div class="msg-con" style="margin-left: -10px;width: 925px"><i class="sui-icon icon-tb-warnfill"></i>${storeEnterAuth.auditOpinion}</div>
							</div>
						</c:if>
					</div>
				</address>
				...
				<form class="sui-form form-inline" method="post" id="inputForm" action="${ctxs}/store/storeEnterInfo/save2.htm">
					...
					<dd class="bs-docs-example">
						...
						<div class="control-group">
							...
							</label> <input type="text" class="input-xlarge" id="compangName" name="companyName" value="${storeEnterAuth.companyName}">
							...
							<span class="sui-dropdown dropdown-bordered select">
								<span class="dropdown-inner">
									<a role="button" data-toggle="dropdown" href="#" class="dropdown-toggle">
										<input type="hidden" name="provinceId" id="provinceId" value="${storeEnterAuth.provinceId}"  onchange="selectCity(this)"><i class="caret"></i>
						...
						<div class="control-group">
							...
							<div class="formPrompt">
								<i class="msg-icon"></i>
								<div class="msg-con">${fns:fy('请输入法人名称')}</div>
							</div>
						</div>
						<div class="control-group">
					...
					<dd class="bs-docs-example">
						...
						<div class="control-group">
							...
							<c:if test="${storeEnterAuth.storeType eq '1'}">${fns:fy('普通店铺')}</c:if>
							<c:if test="${storeEnterAuth.storeType eq '2'}">${fns:fy('品牌旗舰店')}</c:if>
							<c:if test="${storeEnterAuth.storeType eq '3'}">${fns:fy('自营店铺')}</c:if>
							<div class="formPrompt">
								<i class="msg-icon"></i>
								<div class="msg-con">
						...
						<div class="control-group sType">
							...
							<input type="text" class="input-xlarge" id="storeBrand" name="storeBrand" value="${storeEnterAuth.storeBrand}">
							<div class="formPrompt">
								<i class="msg-icon"></i>
						...
						<div class="control-group sType">
							...
							<div class="formPrompt">
								<i class="msg-icon"></i>
								<div class="msg-con">${fns:fy('请压缩图片到大小为1M以内，格式为jpg,jpeg,png,bmp并确保文字清晰、以免上传或审核失败，品牌商标注册证最多只能上传10张。')}</div>
							</div>
						</div>
					</dd>
					<c:if test="${storeEnterAuth.status!='70'}">
