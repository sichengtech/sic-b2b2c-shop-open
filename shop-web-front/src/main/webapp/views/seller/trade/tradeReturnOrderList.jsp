...
<style type="text/css">
	 ...
	 .isProductReceipt{width: 100px;height: 25px;}
...
<body>
	<div class="main-content">
		<div class="sui-row-fluid">
			...
			<dl class="box">
				...
				<address class="sui-row-fluid sui-form form-horizontal screen mb0 ${cookie.fdp_seller_operationTips.value=='0'?'point_hidden':''}" id="point">
					<div class="sui-msg msg-tips msg-block">
						<div class="msg-con">
							<ul>
								<h4>${fns:fy('提示信息')}</h4>
								<li>${fns:fy('此列表显示了商家的所有退货退款信息。')}</li>
				...
				<dd class="table-css">
					<form class="sui-form form-inline" id="searchForm" action="${ctxs}/trade/tradeReturnOrder/tradeReturnOrderList.htm" style="margin: 0 0 7px;" method="get">
						...
						<tbody>
							<c:forEach items="${page.list}" var="tradeReturnOrder" varStatus="index">
								...
								<tr>
									<th colspan="8"> 
										<label>
											<a href="${ctxs}/trade/tradeOrder/tradeOrderDetail.htm?orderId=${tradeReturnOrder.orderId}" style="color:black;"> ${fns:fy('订单编号：')}${tradeReturnOrder.orderId}&nbsp;&nbsp;</a>
											<span>${fns:fy('退款编号：')}${tradeReturnOrder.returnOrderId}</span>
										</label>
									</th>
								</tr>
								<tr>
									...
									<td class="center">
										...
										<input type="hidden" name="deliverProductTime" value="<fmt:formatDate value="${tradeReturnOrder.deliverProductTime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
									</td>
									...
									<td class="center">
										<shiro:hasPermission name="trade:tradeReturnOrder:edit">
											<c:if test="${tradeReturnOrder.businessHandle != '0'  || tradeReturnOrder.systemHandle != '0'}">
												<a class="sui-btn" style="margin-bottom:2px;" href="${ctxs}/trade/tradeReturnOrder/handle1.htm?returnOrderId=${tradeReturnOrder.returnOrderId}&type=1">
													<i class="sui-icon icon-tb-edit"></i>${fns:fy('查看')}
												</a>
											</c:if>
											<c:if test="${tradeReturnOrder.businessHandle == '0' && tradeReturnOrder.systemHandle == '0'}">
												<a href="${ctxs}/trade/tradeReturnOrder/handle1.htm?returnOrderId=${tradeReturnOrder.returnOrderId}&type=1" class="sui-btn btn-primary" style="margin-bottom:2px;">
													<i class="sui-icon icon-touch-edit-rect"></i>${fns:fy('处理')}
												</a>
											</c:if>
											<c:if test="${tradeReturnOrder.status == '40' && (tradeReturnOrder.isProductReceipt == '0' || empty tradeReturnOrder.isProductReceipt)}">
												<a class="sui-btn btn-primary deliverGoodsBtn" style="margin-bottom:2px;" href="javascript:;"  returnOrderId="${tradeReturnOrder.returnOrderId}">
													<i class="sui-icon icon-tb-edit"></i>${fns:fy('收货')}
												</a>
											</c:if>
										</shiro:hasPermission>
										<input type="hidden" name="logisticsTemplateName" value="${tradeReturnOrder.logisticsTemplateName}"/>
										<input type="hidden" name="trackingNo" value="${tradeReturnOrder.trackingNo}"/>
									</td>
								</tr>
							</c:forEach>
							<!-- 没有数据提示开始 -->
	...
	<!-- 收货弹出框开始 -->
	<div id="goodsReceiptModal" tabindex="-1" role="dialog" data-hasfoot="false" class="hide" style="border-radius: 5px;">
		<div class="">
			<div class="modal-body" style="padding: 0;">
				<form id="goodsReceiptForm" name="" action="" method="post">
					<input type="hidden" name="returnOrderId" value="142">
					<table class="sui-table table-bordered" id="delTable">
						<tbody>
							<tr>
								<td class="" style="text-align: right;">${fns:fy('发货时间：')}</td>
								<td><span class="c-success" id="goodsReiceiptTime" name="ordersSn">2017-05-11 10:08:56</span></td>
							</tr>
							<tr>
								<td class="" style="text-align: right;">${fns:fy('物流信息：')}</td>
								<td>
									<div id="logisticsMsg">${fns:fy('申通快递，909090')}</div>
								</td>
							</tr>
							<tr>
								<td class="" style="text-align: right;"><font color="red">*</font>${fns:fy('收货情况：')}</td>
								<td>
									<select class="isProductReceipt" name="isProductReceipt">
										<option value="0">${fns:fy('请选择')}</option>
										<option value="1">${fns:fy('已收到')}</option>
									</select>
								</td>
							</tr>
						</tbody>
					...
					<div class="modal-footer" id="delayReceiptModal-footer">
						<button type="button" data-ok="modal" class="sui-btn btn-primary btn-large" id="deliverGoodsBtn" style="margin-right: 10px;" orderId="">${fns:fy('确定')}</button>
