$(function(){
	...
	$(".searchBtn").click(function(){
		...
		if (minPrice!="" && !re.test(minPrice)) {
			fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> "+fy.getMsg("最小价格请输入正整数或两位以内的小数"),2000);
            return false;
	    }
		//最大值
		var maxPrice=$("input[name='maxPrice']").val();
		if (maxPrice!="" && !re.test(maxPrice)) {
			fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> "+fy.getMsg("最大价格请输入正整数或两位以内的小数"),2000);
            return false;
	    }
