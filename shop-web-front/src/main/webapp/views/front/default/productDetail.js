    ...
    $(".buyNow").click(function(){
		...
		if(status == false){
    		...
    		if(userSellerId== userMemberId){
    			fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> 不能购买自己店铺的商品",2000);
    			return false;
    		}
	    	...
	    	if(type=="1"){
	    		...
	    		if(stock==0){
	    			fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> 库存不足",2000);
	    			return false;
	    		}
	    	...
	    	if(skuArray.length==0){
	    		fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> 请选择规格数量",2000);
				return false;
	    	}
	    	//判断购买数量是否小于起购量
    		...
    		if(purchasingAmount=="" || typeof(purchasingAmount)=="undefined" || totalNum<parseInt(purchasingAmount)){
    			fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> 购买数量必须大于起购量，起购量为"+purchasingAmount,2000);
    			return false;
    		}
    ...
    $(".addCart").click(function(){
		...
		if(status == false){
	    	...
	    	$.ajax({
				...
				success:function(data){
					if(data==1){
						fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:green'></i> 加入购物车成功",2000);
					}else{
						fdp.msg("<i class='fa fa-meh-o' style='font-size:24px;color:red'></i> 加入购物车失败",2000);
					}
				}
	    	});
    	}
				...
				$(layero).find(".askBtn").click(function(){
					if(jsValidate){
						$(layero).find("#askForm").validate({
							...
							messages: {
								"content":{required: "咨询内容不能为空",maxlength:"长度不能超过200字符",},
							},
