(function (){
    ...
    fy.getMsg=function (key, ...rest) {
        ...
        var oldDate=localStorage.getItem("frontDefFyDate");
        if(typeof(oldDate)!="undefined" && oldDate!=null){
            differenceDate=(curDate-oldDate)/(1000*60*60);
        }
        if(differenceDate>=10 || localStorage.getItem("frontFycnMap")==null || localStorage.getItem("frontFyenMap")==null || localStorage.getItem("defaultLocale") == null){
            $.ajax({
                type:"GET",
                url:ctxf+"/fy/poperties.htm",
                data:{},
                async:false,
                //请求成功
                success : function(data) {
                    localStorage.setItem("frontFycnMap",JSON.stringify(data.cnMap));
                    localStorage.setItem("frontFyenMap",JSON.stringify(data.enMap));
                    localStorage.setItem("defaultLocale",data.defaultLocale);
                    localStorage.setItem("frontDefFyDate",new Date().getTime());
                }
            });
        }
        //先从cookike中获取值
        ...
        //中文
        var fycnStr=localStorage.getItem('frontFycnMap');
        var fycnMap=JSON.parse(fycnStr);
        //英文
        var fyenStr=localStorage.getItem("frontFyenMap");
