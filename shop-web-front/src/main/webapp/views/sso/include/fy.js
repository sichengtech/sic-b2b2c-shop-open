(function (){
    ...
    fy.getMsg=function (key, ...rest) {
        ...
        var oldDate=localStorage.getItem("ssoFyDate");
        if(typeof(oldDate)!="undefined" && oldDate!=null){
            differenceDate=(curDate-oldDate)/(1000*60*60);
        }
        if(differenceDate>=10 || localStorage.getItem("ssoFycnMap")==null || localStorage.getItem("ssoFyenMap")==null || localStorage.getItem("defaultLocale") == null){
            $.ajax({
                type:"GET",
                url:ctxsso+"/fy/poperties.htm",
                data:{},
                async:false,
                //请求成功
                success : function(data) {
                    localStorage.setItem("ssoFycnMap",JSON.stringify(data.cnMap));
                    localStorage.setItem("ssoFyenMap",JSON.stringify(data.enMap));
                    localStorage.setItem("defaultLocale",data.defaultLocale);
                    localStorage.setItem("ssoFyDate",new Date().getTime());
                }
            });
        }
        //先从cookike中获取值
        ...
        //中文
        var fycnStr=localStorage.getItem('ssoFycnMap');
        var fycnMap=JSON.parse(fycnStr);
        //英文
        var fyenStr=localStorage.getItem("ssoFyenMap");
