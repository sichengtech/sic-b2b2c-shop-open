(function (){
    ...
    fy.getMsg=function (key, ...rest) {
        ...
        var oldDate=localStorage.getItem("memberFyDate");
        if(typeof(oldDate)!="undefined" && oldDate!=null){
            differenceDate=(curDate-oldDate)/(1000*60*60);
        }
        if(differenceDate>=10 || localStorage.getItem("memberFycnMap")==null || localStorage.getItem("memberFyenMap")==null || localStorage.getItem("defaultLocale") == null){
            $.ajax({
                type:"GET",
                url:ctxm+"/fy/poperties.htm",
                data:{},
                async:false,
                //请求成功
                success : function(data) {
                    localStorage.setItem("memberFycnMap",JSON.stringify(data.cnMap));
                    localStorage.setItem("memberFyenMap",JSON.stringify(data.enMap));
                    localStorage.setItem("defaultLocale",data.defaultLocale);
                    localStorage.setItem("memberFyDate",new Date().getTime());
                }
            });
        }
        //先从cookie中获取值
        ...
        //中文
        var fycnStr=localStorage.getItem('memberFycnMap');
        var fycnMap=JSON.parse(fycnStr);
        //英文
        var fyenStr=localStorage.getItem("memberFyenMap");
