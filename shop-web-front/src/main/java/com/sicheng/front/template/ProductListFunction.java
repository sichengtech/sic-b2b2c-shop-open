...
/**
 * <p>
 * 标题: 自定义函数，ProductListFunction函数
 * </p>
 * <p>
 * 描述: 根据零个或多个条件获取商品列表,不带分页
 * </p>
 * <p>
 ...
 */
public class ProductListFunction implements Function {
    ...
    public Map<String, Object> call(Object[] args, Context ctx) {
        ...
        if (StringUtils.isNotBlank(pids)) {
			...
			if(pidss.length>0){
				for(int i=0;i<pidss.length;i++){
					pidList.add(StringUtils.toLong(pidss[i]));
				}
				wrapper.and("p_id in",pidList);
			}*/
        }
        ...
        //Page<SolrProduct> solrProductPage=service.selectByWhere(new Page<SolrProduct>(1,limit,limit),wrapper);
        Page<SolrProduct> page = new Page<SolrProduct>(1, limit, limit);
