...
//			});
            //翻译
            var fyInfo1=fy.getMsg("文件后缀名有误",s.allowType);
            if(typeof(fyInfo1)=="undefined" || fyInfo1=='' || fyInfo1==null){
            	fyInfo1="仅支持 (" + s.allowType + ") 为后缀名的文件!";
            }
			layer.msg(fyInfo1);
		...
		if (s.fileSize > 0) {
			...
			if (fs > s.fileSize) {
	            //翻译
	            var fyInfo2=fy.getMsg("文件大小超出范围",(fs/1024/1024).toFixed(2),(s.fileSize/1024/1024).toFixed(2));
	            if(typeof(fyInfo2)=="undefined" || fyInfo2=='' || fyInfo2==null){
	            	fyInfo2="当前文件大小 (" + (fs/1024/1024).toFixed(2) + "M) 超过允许的限制值 (" + (s.fileSize/1024/1024).toFixed(2) + "M)！";
	            }
				layer.msg(fyInfo2);
					...
					if (status != "error") {
						...
						if (s.success){
							try{
								...
								//本组件用户写的成功回调函数发生了异常
					            //翻译
					            var fyInfo3=fy.getMsg("上传出现错误",e);
					            if(typeof(fyInfo3)=="undefined" || fyInfo3=='' || fyInfo3==null){
					            	fyInfo3="开发者你好，你编写的success函数生成异常，请你自行修复。"+e;
					            }
