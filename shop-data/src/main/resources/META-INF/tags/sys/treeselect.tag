...
<div class="input-append">
    <div class="input-group m-bot15">
        ...
        <span id="${id}Button" class="input-group-addon ${disabled} ${hideBtn ? 'hide' : ''}"><i
            ...
            submit: function (v, h, f) {
                if (v == "ok") {
                    ...
                    for (var i = 0; i < nodes.length; i++) {
                        ...
                        if (nodes[i].isParent) {
                            continue; // 如果为复选框`，则过滤掉父节点
                        }
                        </c:if><c:if test="${notAllowSelectRoot}">
                        if (nodes[i].level == 0) {
                            top.$.jBox.tip('${fns:fy("不能选择根节点（")}'+nodes[i].name +'${fns:fy("）请重新选择。")}');
                            return false;
                        }
                        </c:if><c:if test="${notAllowSelectParent}">
                        if (nodes[i].isParent) {
                            top.$.jBox.tip('${fns:fy("不能选择父节点（")}'+nodes[i].name +'${fns:fy("）请重新选择。")}');
                            return false;
                        }
                        </c:if><c:if test="${not empty module && selectScopeModule}">
                        if (nodes[i].module == "") {
                            top.$.jBox.tip('${fns:fy("不能选择公共模型（")}'+nodes[i].name +'${fns:fy("）请重新选择。")}');
                            return false;
                        } else if (nodes[i].module != "${module}") {
                            top.$.jBox.tip('${fns:fy("不能选择当前栏目以外的栏目模型，请重新选择。")}');
                            return false;
                        }
                        </c:if>
                        ids.push(nodes[i].id);
