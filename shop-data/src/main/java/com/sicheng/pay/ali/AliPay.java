 ...
 *
 * @author cailong
 * @version 2018年3月26日 下午2:51:35
 */
@Service("aliPay")
public class AliPay implements ShopPay {
    ...
    private final String PAY_MAY_NUM = "ali";//支付方式编号
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:shop商户多个订单号不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:shop商户多个订单价格不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:商户订单号不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:付款总金额不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:支付方式id不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单参数错误:订单标题不能为空");
//			return resultMap;
...
//			resultMap.put("returnCode","FAIL");
//			resultMap.put("returnMsg","支付宝统一下单出现问题");
//			return resultMap;
...
//		model.setOutTradeNo("1522395542");//测试号
//		model.setTradeNo("2018033021001004610200351709");//测试号
...
//	@Override
    ...
    public Object downloadBill(Map<String, Object> map) throws PayException {
        ...
        logger.info("支付宝对账单下载地址-billType：" + billType);
        //账单日期
        String billDate = (String) map.get("bill_date");
        logger.info("支付宝对账单下载地址-billDate：" + billDate);
        if (StringUtils.isBlank(billType)) {
            throw new PayException("账单类型不能为空");
        }
        if (StringUtils.isBlank(billDate)) {
            throw new PayException("账单日期不能为空");
        }
        ...
        try {
            ...
            logger.info("支付宝对账单下载地址：" + response.getBody());
            if (!response.isSuccess()) {
                throw new PayException("查询对账单地址失败");
            }
            //将接口返回的对账单下载地址传入urlStr
            String urlStr = response.getBillDownloadUrl();
            resultMap.put("returnCode", "SUCCESS");
            resultMap.put("returnMsg", "查询对账单地址成功");
            resultMap.put("filePath", urlStr);
            return resultMap;
        } catch (AlipayApiException e) {
            logger.info("下载支付宝账单地址失败:", e);
            throw new PayException("查询账单地址失败，服务器发生异常");
        }
    }
