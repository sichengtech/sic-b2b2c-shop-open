$(function(){
	 ...
	 * 评论图片点击预览
	 * */
	$("body").delegate(".comment-img img","click",function(){
		//给图片添加尺寸属性
		//auto_data_size($(this).parent().find("img"));
		//当前点击的第几张图
        ...
        $(this).parent().find("img").each(function(){
			...
			var src=$(this).attr("src").substring(0,srcIndex);
			var item = {
                src:src,
	 ...
	 * 给图片添加尺寸属性
	 * */
	function auto_data_size(content){
    ...
    function initCommentDate(grade,page,size,me){
    	...
    	$.ajax({
        	type: 'GET',
            url:ctxw+'/api/v1/product/comment/page.htm',
            data:{"pid":pid,"pageNo":page,"pageSize":size,"grade":grade,"type":"0"},
            dataType: 'json',
            success: function(data){
            	...
            	setTimeout(function(){
               		...
               		auto_data_size('.comment-items-grade'+grade);
	...
	function getCommentImg(commentIds){
		...
		$.ajax({
	    	type: 'GET',
	        url:ctxw+'/api/v1/product/comment/image/list.htm',
	...
	function getCommentReply(commentIds){
		...
		$.ajax({
	    	type: 'GET',
	        url:ctxw+'/api/v1/product/comment/page.htm',
            data:{"pid":pid,"replyIds":commentIds,"type":"2"},
	        dataType: 'json',
	        async: false,
	        success: function(data){
	        	if(data==null || data.status!=200 || typeof(data.data)=="undefined" || data.data==null){
	...
	function getAddComment(commentIds){
		...
		$.ajax({
	    	type: 'GET',
	        url:ctxw+'/api/v1/product/comment/page.htm',
            data:{"pid":pid,"replyIds":commentIds,"type":"1"},
	        dataType: 'json',
	        async: false,
	        success: function(data){
	        	if(data==null || data.status!=200 || typeof(data.data)=="undefined" || data.data==null){
