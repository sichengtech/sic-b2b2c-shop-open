(function (){
    ...
    fy.getMsg=function (key, ...rest) {
        ...
        var oldDate=localStorage.getItem("wapFyDate");
        if(typeof(oldDate)!="undefined" && oldDate!=null){
            differenceDate=(curDate-oldDate)/(1000*60*60);
        }
        if(differenceDate>=10 || localStorage.getItem("wapFycnMap")==null || localStorage.getItem("wapFyenMap")==null || localStorage.getItem("defaultLocale") == null){
            $.ajax({
                type:"GET",
                url:ctxw+"/fy/poperties.htm",
                data:{},
                async:false,
                //请求成功
                success : function(data) {
                    localStorage.setItem("wapFycnMap",JSON.stringify(data.cnMap));
                    localStorage.setItem("wapFyenMap",JSON.stringify(data.enMap));
                    localStorage.setItem("defaultLocale",data.defaultLocale);
                    localStorage.setItem("wapFyDate",new Date().getTime());
                }
            });
        }
        //先从cookike中获取值
        ...
        //中文
        var fycnStr=localStorage.getItem('wapFycnMap');
        var fycnMap=JSON.parse(fycnStr);
        //英文
        var fyenStr=localStorage.getItem("wapFyenMap");
